...
if ( ! class_exists( 'SODAThemesFeaturedPost' ) ) {
	class SODAThemesFeaturedPost {
		...
		public function featured_post_states( $states ) {
			global $post;
			if ( ! empty( $post ) && get_post_meta( $post->ID, '_is_featured', true ) === 'yes' ) {
				$states[] = esc_html__( 'Featured', 'sodathemes' );
			}
			return $states;
		}

		public function manage_posts_columns( $columns ) {
			$columns[ 'featured' ] = esc_html__( 'Featured', 'sodathemes' );
			return $columns;
		}
		...
		public function edit_screen_featured_ui() {
			if ( is_admin() ) {
				...
				echo '<label for="featured" title="' . esc_attr__( 'If checked, this is marked as featured.', 'sodathemes' ) . '">' . "\n";
				echo esc_html__( 'Featured?', 'sodathemes' ) . ' <input id="featured"" type="checkbox" value="yes" ' . checked( get_post_meta( $post->ID, '_is_featured', true ), 'yes', false ) . ' name="featured" /></label></div>' . "\n";
			}
		}
	...
	SODAThemesFeaturedPost::instance();
}